...
class StorageManager:
    ...
    def __init__(
        ...
        Args:
            ...
            timezone: 时区配置
            mysql_config: MySQL 同步配置（enabled/host/port/user/password/database），
                          可选，作为附加同步器，不影响 SQLite 主存储
        """
    ...
    def _get_mysql_backend(self):
        """获取 MySQL 同步后端（若启用）"""
        if self._mysql_backend is not None:
            return self._mysql_backend

        mysql_cfg = self.mysql_config or {}
        if not mysql_cfg.get("enabled", False):
            self._mysql_backend = None
            return None
        ...
        try:
            ...
            self._mysql_backend = MySQLSyncBackend(
            ...
            print("[存储管理器] 启用 MySQL 同步（与 SQLite 并存）")
        except Exception as e:
            print(f"[存储管理器] MySQL 同步后端初始化失败: {e}")
            self._mysql_backend = None
    ...
    def save_news_data(self, data: NewsData) -> bool:
        """保存新闻数据（SQLite 主存储 + 可选的 MySQL 同步）"""
        success = self.get_backend().save_news_data(data)
        # MySQL 同步（附加，不影响主存储结果）
        mysql_backend = self._get_mysql_backend()
        if mysql_backend is not None:
            mysql_backend.sync_news_data(data)
        return success

    def save_rss_data(self, data: RSSData) -> bool:
        """保存 RSS 数据（SQLite 主存储 + 可选的 MySQL 同步）"""
        success = self.get_backend().save_rss_data(data)
        # MySQL 同步（附加，不影响主存储结果）
        mysql_backend = self._get_mysql_backend()
        if mysql_backend is not None:
            mysql_backend.sync_rss_data(data)
        return success
